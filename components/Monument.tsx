...
import { generateAtlasTexture, generateNormalMap, CHAR_SET } from './TextureGen';
...
const injectAtlasShader = (shader: THREE.Shader) => {
  const cols = CHAR_SET.length.toFixed(1); // Dynamic column count based on charset
        ...
        for (let z = -2; z <= 2; z++) {
          ...
          if (shapeNoise > 0.1) {
             cubes.push({
               ...
               y: y * CONFIG.cubeSize,
               z: z * CONFIG.cubeSize,
               atlasIndex: Math.floor(Math.random() * CHAR_SET.length),
